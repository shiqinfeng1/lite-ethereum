...
import (
	...
	"github.com/shiqinfeng1/lite-ethereum/rlp"
)
...
func ListenV4(c UDPConn, ln *enode.LocalNode, cfg Config) (*UDPv4, error) {
	...
	log.Println("Create Discover Table ok .")
	for _, n := range cfg.Bootnodes {
		log.Println("BootNode: ", n.String())
	}
...
// DumpTable Dump Discover Table
func (t *UDPv4) DumpTable() string {
	str, _ := json.MarshalIndent(t.tab.DumpAllNodes(), "", "    ")
	return string(str)
}
...
func (t *UDPv4) lookupWorker(n *node, targetKey encPubkey, reply chan<- []*node) {
	...
	if err == errClosed {
		...
		//log.Println("Findnode failed", "id", n.ID(), "failcount", fails, "err", err)
		if fails >= maxFindnodeFailures {
			// log.Println("Too many findnode failures, dropping", "id", n.ID(), "failcount", fails)
			t.tab.delete(n)
		}
	} else if fails > 0 {
...
func (t *UDPv4) findnode(toid enode.ID, toaddr *net.UDPAddr, target encPubkey) ([]*node, error) {
	// log.Println("UDPv4 Send Packet<Findnode> To", toid, "At", toaddr)
	...
	rm := t.pending(toid, toaddr.IP, p_neighborsV4, func(r interface{}) (matched bool, requestDone bool) {
		...
		for _, rn := range reply.Nodes {
			...
			if err != nil {
				// log.Println("Invalid neighbor node received", "ip", rn.IP, "addr", toaddr, "err", err)
				continue
			}
...
func (t *UDPv4) RequestENR(n *enode.Node) (*enode.Node, error) {
	log.Println("[UDPv4.RequestENR]", n, n.ID())
	...
	for {
		...
		case r := <-t.gotreply:
			...
			for el := plist.Front(); el != nil; el = el.Next() {
				...
				//log.Println("[[P2P GotReply]] expect.from", p.from, "real.from ", r.from, "expect.ptype", p.ptype, "real.data.kind()", r.data.kind(), "expect.ip", p.ip, "real.ip", r.ip)
...
func (t *UDPv4) encode(priv *ecdsa.PrivateKey, req packetV4) (packet, hash []byte, err error) {
	...
	if err := rlp.Encode(b, req); err != nil {
		log.Println(fmt.Sprintf("Can't encode %s packet", name), "err", err)
		return nil, nil, err
	}
	packet = b.Bytes()
	sig, err := crypto.Sign(crypto.Keccak256(packet[headSize:]), priv)
	if err != nil {
		log.Println(fmt.Sprintf("Can't sign %s packet", name), "err", err)
		return nil, nil, err
	}
...
func (t *UDPv4) readLoop(unhandled chan<- ReadPacket) {
	...
	for {
		...
		if netutil.IsTemporaryError(err) {
			...
			log.Println("Temporary UDP read error", "err", err)
			continue
		} else if err != nil {
			// Shut down the loop for permament errors.
			if err != io.EOF {
				log.Println("UDP read error", "err", err)
			}
			return
		}
...
func (t *UDPv4) handlePacket(from *net.UDPAddr, buf []byte) error {
	...
	if err != nil {
		log.Println("Bad discv4 packet", "addr", from, "err", err)
		return err
	}
	fromID := fromKey.id()
	if err == nil {
	...
	//log.Println("UDPv4 Receive << "+packet.name(), "fromaddr", from, "err", err)
	if err == nil {
...
func (t *UDPv4) ensureBond(toid enode.ID, toaddr *net.UDPAddr) {
	//log.Println("[ensureBond]Send Ping To Make A Valid Endpoint Proof.", "toid:", toid)
